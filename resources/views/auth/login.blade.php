    ...
    <div class="container with_login_form">
        <div class="form_cont">
            ...
            <form action="{{ route('login') }}" method="POST">
                {{ csrf_field() }}
                @if ($errors->has('error'))
                    <div class="alert alert-danger" style="font-size: 12px; height: 20px; padding: 0; vertical-align: middle; text-align: center; margin-bottom: 0;">
                        {{ $errors->first('error') }}
                    </div>
                @endif
                <div class="form-group">
                    <label for="email">User Name</label>
                    <input id="email" name="email" type="email" placeholder="Enter Email or User Name" required value="{{ old('email') }}" class="form-control">
                </div>
                <div class="form-group">
                    <label for="password">Password</label>
                    <input id="password" name="password" type="password" placeholder="Enter Password" required value="{{ old('password') }}" class="form-control">
                </div>
                <button type="submit" class="btn btn-default login_btn">LOGIN</button>
                <div align="center" id="forgot_pass">
                    <span style="color: #919191;">Forgot Password?</span>
