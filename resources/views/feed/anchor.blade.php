...
<div class="col-xs-12 @if($count%2 != 0)left_feed_block @else right_feed_block @endif pid_help" data-count="{{ $count }}" data-pid="{{ $feed->project_id }}" id="anchor">
    <div class="@if($count%2 != 0)message_box_left @else message_box_right col-md-offset-6 @endif col-xs-6">
        ...
        <div class="message arrow_box">
            <p style="margin: 0;" class="anchors_confirmed">
                Anchors Confirmed <a href="#" id="anchors_confirmed">{{ $feed->project->url }}</a> <br>
                <i class="mdi mdi-link-variant"></i><a href="{{ route('link_anchor', $feed->project_id) }}">View Detail</a>
            </p>
        </div>
    </div>
</div>
