...
<div class="col-xs-12 @if($count%2 != 0)left_feed_block @else right_feed_block @endif pid_help" data-count="{{ $count }}" data-pid="{{ $feed->project_id }}" id="order">
    <div class="@if($count%2 != 0)message_box_left @else message_box_right col-md-offset-6 @endif col-xs-6">
        ...
        <!--<div class="time_ago"><i class="mdi mdi-clock"></i></div>-->
        ...
        <div class="message arrow_box">
            <p style="margin: 0;" class="new_order">
                New Order <br>
                Order Datail <br>
                Order No: <span id="order_no">{{ $feed->table_id }}</span> <br>
                Order Type: <span id="order_type">{{ $feed->project->orders()->find($feed->table_id)->products()->first()->title }}</span> <br>
                <i class="mdi mdi-link-variant"></i><a href="{{ route('orderDownload', $feed->project->orders()->find($feed->table_id)->id) }}">Full Detail</a>
            </p>
        </div>
    </div>
</div>
