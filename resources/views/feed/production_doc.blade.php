...
<div class="col-xs-12 @if($count%2 != 0)left_feed_block @else right_feed_block @endif pid_help" data-count="{{ $count }}" data-pid="{{ $feed->project_id }}" id="production_doc">
    <div class="@if($count%2 != 0)message_box_left @else message_box_right col-md-offset-6 @endif col-xs-6">
        ...
        <div class="message arrow_box doc-massage-mm">
            <p style="margin: 0;" class="doc_uploaded">
                Document Uploaded <br>
                <i class="mdi  mdi-file-document"></i><a href="{{ route('download.doc.production', $feed->table_id) }}">{{ $feed->project->productions()->find($feed->table_id)->content }}</a>
            </p>
        </div>
    </div>
</div>
