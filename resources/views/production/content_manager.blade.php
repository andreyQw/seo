            ...
            <table class="table table-hover table-responsive table-bordered assined context">
                ...
                @if(!$production->writer_id)
                    <tr  class="gradeA">
                        ...
                        <td>
                            <select class="form-control feedback" style="border: 0px; padding: 0px; " name="writer" production_id="{{$production->id}}">
                                @if($production->writer_id)
                                    <option default>{{$production->writer->user->first_name}} {{$production->writer->user->last_name}}</option>
                                @else
                                    <option default>Choose Writer</option>
                                @endif
                                @foreach($writers as $writer)
                                @if($production->writer_id != $writer->id)
                                <option style="border: 0px;" value="{{$writer->id}}" >{{$writer->user->first_name}} {{$writer->user->last_name}}</option>
                                @endif
                                @endforeach
                            </select>
                        </td>
...
@foreach($writers as $writer)
        <div class="col-lg-12 marked">
            ...
            <table class="table table-responsive table-hover table-bordered status " id="writer{{$writer->id}}" >
                <thead style="background-color: #f15e33; color: white">
                <tr>
                    <th>Partner Name</th>
                    <th class="noExl">Priority</th>
                ...
                <tbody style="background-color: white" >
                @foreach($productions as $production)
                    @if($production->writer_id == $writer->id && $production->content_written != 'Finished')
                    <tr  class="gradeA">
                        <td>{{$production->partner->domain}}</td>
                            <td >
                        ...
                        <td>
                            <textarea class="content-cell feedback" production_id="{{$production->id}}" name="topic">{{$production->topic}}</textarea>
                        </td>
                        <td>
                            <select class="form-control feedback approve" style="border: 0px; padding: 0px; " name="content_written" production_id="{{$production->id}}">
                                <option style="border: 0px;" >{{$production->content_written}}</option>
                                @if($production->content_written == 'Finished')
                                {{--<option style="border: 0px;" >Finished</option>--}}
                                <option style="border: 0px;">Working</option>
                                <option style="border: 0px;">Not Working</option>
                                @endif
                                @if($production->content_written == 'Working')
                                <option style="border: 0px;" >Finished</option>
                                {{--<option style="border: 0px;">Working</option>--}}
                                <option style="border: 0px;">Not Working</option>
                                @endif
                                @if($production->content_written == 'Not Working')
                                <option style="border: 0px;" >Finished</option>
                                <option style="border: 0px;">Working</option>
                                {{--<option style="border: 0px;">Not Working</option>--}}
                                @else
...
//                dom: 'f<"toolbar">t'
            ...
            $('.feedback').change(function () {
                ...
                if (y == 'writer') {
                    ...
                    $("#writer"+x).append($(this).parents("tr").show());
                    $.ajax({
                    type: "POST",
                    url: "{{route('production.topic_ajax')}}",
                    data: {x:x,name:$(this).attr('name'),production_id:$(this).attr('production_id'), _token:"{{ csrf_token() }}"},
                    success: function(data) {
                        console.log(data);
                    },
                });
                    $(this).attr('name', 'content_written');                    
                    $(this).html('<option style="border: 0px;" >Not Working</option>' +
                                '<option style="border: 0px;">Working</option>' +
                                '<option style="border: 0px;">Finished</option>');
                    var x = 'Not Working';
                }
